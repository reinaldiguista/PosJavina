        ...
        <div class="box">
            <div class="box-header with-border">
                <table>
                    ...
                    <tr>
                        <td>No. ref </td>
                        <td>&nbsp; : {{ $invoice->number_ref }}</td>
                    </tr>
                    <tr>
                        <td>Jumlah Invoice </td>
                        <td>&nbsp;  : {{ format_uang($invoice->invoice_amount) }}</td>
                    </tr>
                    <tr>
                        <td>Sisa Terhutang </td>
                        <td>&nbsp;  : {{ format_uang($invoice->invoice_debt) }}</td>
                    </tr>
                    <tr>
                        <td>Tenggat Waktu </td>
                        <td>&nbsp;  : {{ tanggal_indonesia($invoice->due_date) }}</td>
                    </tr>
                </table>
            </div>
                ...
                <div class="col-lg-5">
                    <div class="input-group">
                        ...
                        <span class="input-group-btn">
                            <button onclick="addForm('{{ route('invoice_pay.store') }}')" class="btn btn-success btn-lg "><i class="fa fa-plus-circle"></i> Tambah</button>
                        </span>
                    </div>
                </div>
            </div>
            ...
            <div class="box-footer">
                <a onclick="bug()" href="{{ route('invoice.index') }}" class="btn btn-warning btn-sm btn-flat pull-left">
...
@includeIf('invoice_pay.invoice')
@includeIf('invoice_pay.form')
...
<script>
    ...
    $(function () {
        ...
        table = $('.table-invoice').DataTable({
            ...
            autoWidth: false,
            ajax: {
                url: '{{ route('invoice_pay.data', $id_invoice) }}',
            },
            columns: [
        ...
        $(document).on('change', '.discount', function () {
            let id = $(this).data('id');
            let transaction = $('#id_invoice').val(); 
    ...
    function tambahProduk() {
        ...
        console.log(id_invoice);
            ...
            $.post(`{{ url('/stock_detail/addProduct') }}/${sku}/${count}/${id_invoice}`, {
                    '_token': $('[name=csrf-token]').attr('content'),
                    '_method': 'post',
                })
                .done(response => {
    ...
    function tes(id, cb) {
        ...
        function cancel() {
            ...
            $.get(`{{ url('/invoice/hapus') }}/${invoice_id}/`, {
                    '_token': $('[name=csrf-token]').attr('content'),
                    '_method': 'get'
                })
                .done(response => {
    ...
    function addForm(url) {
        ...
        $('#modal-form .modal-title').text('Tambah Invoice');
